...
int bfs_search(std::vector<std::string> grid, int x, int y)
{
    ...
    for (auto it = visited.begin(); it != visited.end(); it++) {
        node node = it->second;
        for (int i=0; i<4; i++) {
            std::array<int, 2> skip = add_vecs(add_vecs(node.pos, moves[i]), moves[i]);
            if (visited.find(skip) != visited.end()) {
                int dist_saved = visited[skip].dist - node.dist - 2;
//                std::cout << "dist_saved: " << dist_saved << '\n';
                if (dist_saved >= 100)
                    out++;
            }
        }
